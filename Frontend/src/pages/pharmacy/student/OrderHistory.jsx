...
const OrderHistory = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case "DELIVERED": return 'bg-[#e8f7f5] text-emerald-700 border-emerald-100';
      case "CANCELLED": return 'bg-rose-50 text-rose-700 border-rose-100';
      case "PENDING": return 'bg-amber-50 text-amber-700 border-amber-100';
      default: return 'bg-[#e8f4f8] text-blue-700 border-blue-100';
    }
  };
      ...
      <div className="max-w-5xl mx-auto">
        <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div>
            ...
            <p className="text-secondary-text">Track and manage your orders and prescriptions</p>
          </div>
        ...
        <div className="space-y-6">
                    ...
                    <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                      <div className="flex items-start gap-6">
                        ...
                        <div>
                          <div className="flex items-center gap-3 mb-1">
                            <h3 className="text-lg font-bold text-primary-text">Order {order.id}</h3>
                            <span className={cn(
                          ...
                          <div className="flex -space-x-3 overflow-hidden">
                            {order.items.map((item, idx) => {
                              ...
                              return (
                                <div key={idx} className="inline-block h-10 w-10 rounded-full ring-4 ring-white bg-[#e6f0f4] overflow-hidden">
                                  <img src={med?.image} alt={med?.name} className="h-full w-full object-cover" referrerPolicy="no-referrer" />
                                </div>
                              );
                            })}
                            {order.items.length > 3 && (
                              <div className="flex items-center justify-center h-10 w-10 rounded-full ring-4 ring-white bg-slate-200 text-[10px] font-bold text-secondary-text">
                                +{order.items.length - 3}
                              </div>
                            )}
                          </div>
                      ...
                      <div className="flex flex-col items-end gap-4">
                        <div className="text-right">
                          ...
                          <p className="text-2xl font-bold text-primary-text">${order.total.toFixed(2)}</p>
                        </div>
                ...
                >
                  <div className="flex flex-col md:flex-row justify-between gap-6">
                    <div className="flex gap-6">
                      <div className="w-20 h-28 bg-[#e6f0f4] rounded-2xl overflow-hidden shadow-inner border border-slate-200 relative shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-xl font-bold text-primary-text truncate">{p.studentName}</h3>
                          <span className={cn(
                            "px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border",
                            p.status === "PENDING" ? "bg-amber-50 text-amber-700 border-amber-100" : 
                            p.status === "APPROVED" ? "bg-[#e8f7f5] text-emerald-700 border-emerald-100" : 
                        ...
                        <div className="flex items-center gap-4 text-xs font-bold text-secondary-text/80">
          ...
          {(viewMode === 'Orders' ? filteredOrders : filteredPrescriptions).length === 0 && (
            <div className="text-center py-20 bg-white rounded-3xl border border-slate-200 border-dashed">
              ...
              <p className="text-secondary-text">You haven't placed any {viewMode.toLowerCase()} in this category yet.</p>
              <Link 
